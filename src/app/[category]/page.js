  ...
  useEffect(() => {
    ...
    async function fetchImages() {
      const res = await fetch(
        `http://46.235.8.12:1337/api/images?populate=*&filters[categories][Title][$eq]=${encodeURIComponent(
          category
        )}`
      );
        ...
        {images.map((item, index) => {
          ...
          const imageUrl = image?.url
            ? image.url.startsWith("http")
              ? image.url
              : `http://46.235.8.12:1337${image.url}`
            : null;
            ...
            <Image
              src={
                currentImage.image.url.startsWith("http")
                  ...
                  : `http://46.235.8.12:1337${currentImage.image.url}`
              }
